...
const Menu = () => {
    ...
    return (
        <div className='py-36 px-32'>
            <h2 className='text-5xl lg:text-7xl leading-snug font-thin mb-5 font-cabin italic text-blue-950 text-center'>Thực đơn</h2>

            {Object.entries(groupedDishes).map(([categoryName, dishes]) => (
                <section key={categoryName} className="mb-12">
                    <h2 className="text-3xl font-bold font-inter mb-10 pl-4 text-blue-900">{categoryName}</h2>
                        ...
                        {dishes.map((item) => (
                            <Link key={item.id} to={`/menu/${item.id}`}>
                                <div className="min-w-[250px] bg-white rounded-lg shadow-lg p-4">
                                    ...
                                    <p className="text-lg text-blue-900 font-cabin">{item.price.toLocaleString('vi-VN')} đ</p>
                                </div>
                            </Link>
                        ))}
                    </div>
